...
/* Cloudflare mirror configuration */
extern char *mydns_cf_soa_table_name;
extern char *mydns_cf_rr_table_name;
extern char *mydns_cf_default_ns;
extern char *mydns_cf_default_mbox;
extern int mydns_cf_enabled;

void mydns_set_cf_soa_table_name(const char *name);
void mydns_set_cf_rr_table_name(const char *name);
void mydns_set_cf_default_ns(const char *name);
void mydns_set_cf_default_mbox(const char *name);
void mydns_update_cloudflare_state(void);
...
extern int		mydns_rr_srv_values(const MYDNS_RR *, uint16_t *, uint16_t *, uint16_t *, char **);
extern int		mydns_rr_rp_values(const MYDNS_RR *, char **, char **);
extern int		mydns_rr_naptr_values(const MYDNS_RR *, uint16_t *, uint16_t *, char **, char **, char **, char **);
...
/* Allow up to four SQL hosts to be configured */
#define			SQL_MAX_DB_HOSTS	4
extern void		sql_configure_hosts(const char **hosts, size_t count, const char *policy);
extern const char	*sql_active_host(void);
...
extern void		db_output_cloudflare_tables(void);
