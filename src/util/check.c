...
static int
srv_label_valid(const char *label, int allow_hyphen) {
  const unsigned char *cp;

  if (!label || !*label)
    return 0;
  for (cp = (const unsigned char *)label; *cp; cp++) {
    if (allow_hyphen) {
      if (!(isalnum(*cp) || *cp == '-'))
	return 0;
    } else {
...
static void
check_rr_srv_owner(void) {
  char *owner, *cursor, *label;
  int service_seen = 0, proto_seen = 0, domain_seen = 0;
  ...
  cursor = owner;
  while ((label = strsep(&cursor, "."))) {
    if (!*label)
      continue;
    if (!service_seen) {
      if (label[0] != '_' || !srv_label_valid(label + 1, 1))
	rrproblem(_("SRV owner must start with _service"));
      service_seen = 1;
      continue;
    }
    if (!proto_seen) {
      if (label[0] != '_')
	rrproblem(_("SRV owner must include protocol label prefixed with '_'"));
      else if (strcasecmp(label, "_tcp") && strcasecmp(label, "_udp") && strcasecmp(label, "_sctp"))
	rrproblem(_("SRV protocol label should be _tcp, _udp or _sctp"));
      proto_seen = 1;
      continue;
    }
    domain_seen = 1;
  ...
  if (!domain_seen)
    rrproblem(_("SRV owner must be of the form _service._proto.example.com"));

  RELEASE(owner);
}

static int
parse_srv_uint16(const char *token, const char *field, int allow_zero) {
  char *end = NULL;
  unsigned long value;

  if (!token || !*token) {
    rrproblem(_("Missing %s in SRV record"), field);
    return (-1);
  }
  errno = 0;
  value = strtoul(token, &end, 10);
  if (*end != '\0' || errno == ERANGE) {
    rrproblem(_("Non-numeric %s `%s' in SRV record"), field, token);
    return (-1);
  }
  if (value > UINT16_MAX || (!allow_zero && value == 0)) {
    rrproblem(_("Value in `%s' out of range"), field);
    return (-1);
  }
  return ((int)value);
}
...
static void
check_rr_srv(void) {
  char weight_str[32], port_str[32], targetbuf[DNS_MAXNAMELEN + 1];
  uint32_t aux = rr->aux;
  uint16_t priority = (uint16_t)(aux & 0xFFFF);

  check_rr_srv_owner();

  if (sscanf(data, "%31s %31s %255s", weight_str, port_str, targetbuf) != 3) {
    rrproblem(_("SRV data must be `weight port target'"));
    return;
  }

  parse_srv_uint16(weight_str, _("weight"), 1);
  parse_srv_uint16(port_str, _("port"), 0);

  if (aux > UINT16_MAX)
    rrproblem(_("SRV priority must fit in 16 bits"));

  if (!*targetbuf)
    rrproblem(_("SRV target host missing"));
  else {
    char *target = STRDUP(targetbuf);
    if (target) {
      EXPAND_DATA(target);
      check_name(target, "rr.data (SRV target)", 1, 0);
      RELEASE(target);
    }
  }
}
...
static int
naptr_validate_uint16(const char *token, const char *label) {
  char *end = NULL;
  unsigned long value;

  if (!token || !*token) {
    rrproblem(_("NAPTR %s field is empty"), label);
    return (-1);
  }
  errno = 0;
  value = strtoul(token, &end, 10);
  if (*end != '\0' || errno == ERANGE) {
    rrproblem(_("NAPTR %s `%s' is not numeric"), label, token);
    return (-1);
  }
  if (value > UINT16_MAX) {
    rrproblem(_("NAPTR %s `%s' exceeds 65535"), label, token);
    return (-1);
  }
  return ((int)value);
}
...
static void
naptr_validate_flags(const char *flags) {
  const unsigned char *cp;

  if (!flags)
    return;
  for (cp = (const unsigned char *)flags; *cp; cp++) {
    if (!(isalnum(*cp))) {
      rrproblem(_("NAPTR flags contain invalid character `%c'"), *cp);
      break;
    }
  }
}
...
static void
naptr_validate_service(const char *service) {
  const unsigned char *cp;

  if (!service || !*service)
    return;
  for (cp = (const unsigned char *)service; *cp; cp++) {
    if (isspace(*cp))
      rrproblem(_("NAPTR service cannot contain spaces"));
    else if (!(isalnum(*cp) || *cp == '.' || *cp == '+' || *cp == '-' || *cp == '_'))
      rrproblem(_("NAPTR service contains invalid character `%c'"), *cp);
  }
}

static void
naptr_validate_regex(const char *regex) {
  int delimiters = 0;
  const unsigned char *cp;
  unsigned char delim;
  ...
  if (!regex || !*regex)
    return;
  delim = (unsigned char)regex[0];
  if (isspace(delim)) {
    rrproblem(_("NAPTR regexp delimiter cannot be whitespace"));
    return;
  }
  for (cp = (const unsigned char *)(regex + 1); *cp; cp++) {
    if (*cp == delim)
      delimiters++;
  }
  if (delimiters < 2)
    rrproblem(_("NAPTR regexp must include pattern, replacement, and flag sections"));
}
...
static void
check_rr_naptr(void) {
  char *data_copy = NULL, *cursor = NULL;
  char *order = NULL, *preference = NULL;
  char *flags = NULL, *service = NULL;
  char *regex = NULL, *replacement = NULL;
  ...
  if (!strsep_quotes2(&cursor, &order))
    goto missing_field;
  if (naptr_validate_uint16(order, _("order")) < 0)
    goto cleanup;

  if (!strsep_quotes2(&cursor, &preference))
    goto missing_field;
  if (naptr_validate_uint16(preference, _("preference")) < 0)
    goto cleanup;

  if (!strsep_quotes2(&cursor, &flags))
    goto missing_field;
  naptr_validate_flags(flags);

  if (!strsep_quotes2(&cursor, &service))
    goto missing_field;
  naptr_validate_service(service);

  if (!strsep_quotes2(&cursor, &regex))
    goto missing_field;
  naptr_validate_regex(regex);

  if (!strsep_quotes2(&cursor, &replacement))
    goto missing_field;
  if (replacement && strcmp(replacement, "."))
    check_name(replacement, "rr.data (replacement)", 1, 0);

  goto cleanup;

missing_field:
  rrproblem(_("Malformed NAPTR record (missing fields)"));
...
check_rr(void) {
  ...
  case DNS_QTYPE_RP:							/* Data: Responsible person */
    {
      char *mbox = NULL, *txt = NULL;
      if (mydns_rr_rp_values(rr, &mbox, &txt) < 0) {
	rrproblem(_("Invalid RP record"));
      } else {
	check_name(mbox, "rr.data (mbox)", 1, 0);
