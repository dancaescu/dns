...
axfr_connect(char *hostportp, char **hostnamep) {
  ...
  if (hostport[0] == '[') {				/* IPv6 literal with brackets */
    char *end = strchr(hostport, ']');
    if (!end)
      Errx(_("invalid IPv6 host syntax: %s"), hostportp);
    *end = '\0';
    rem_hostname = hostport + 1;
    if (end[1] == ':' || end[1] == '+') {
      port = atoi(&end[2]);
      if (!port) port = 53;
    }
  } else {
    char *plus = strrchr(hostport, '+');
    if (plus) {
      *plus++ = '\0';
      if (*plus)
	port = atoi(plus);
      ...
      if (colon && colon == strrchr(hostport, ':') && strchr(hostport, ':') == colon) {
	*colon++ = '\0';
	if (*colon)
	  port = atoi(colon);
      }
  ...
  *hostnamep = rem_hostname;
  ...
  if ((gaierr = getaddrinfo(rem_hostname, portbuf, &hints, &res)) != 0)
    Errx("%s: %s", rem_hostname, gai_strerror(gaierr));
  ...
  for (rp = res; rp; rp = rp->ai_next) {
    ...
    Warn("%s (%s)", rem_hostname, addrbuf[0] ? addrbuf : _("unknown host"));
    close(fd);
  }
  if (res)
    freeaddrinfo(res);
