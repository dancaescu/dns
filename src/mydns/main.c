...
/*  puts("----------------------------------------------------------------------------78");  */
    ...
    puts(_("      --host2=HOST        optional secondary database host"));
    puts(_("      --host3=HOST        optional tertiary database host"));
    puts(_("      --host4=HOST        optional quaternary database host"));
    puts(_("      --host-policy=POL   host selection policy (sequential|round-robin|least-used)"));
...
cmdline(int argc, char **argv) {
  ...
  int	do_create_cf_tables = 0;
  struct option const longopts[] = {
    {"background",		no_argument,			NULL,	'b'},
    {"conf",			required_argument,		NULL,	'c'},
    {"create-tables",		no_argument,			NULL,	0},
    {"create-cloudflare-tables", no_argument,			NULL,	0},
    {"dump-config",		no_argument,			NULL,	0},

    {"database",		required_argument,		NULL,	'D'},
    {"host",			required_argument,		NULL,	'h'},
    {"host2",			required_argument,		NULL,	0},
    {"host3",			required_argument,		NULL,	0},
    {"host4",			required_argument,		NULL,	0},
    {"host-policy",		required_argument,		NULL,	0},
  ...
  while ((optc = getopt_long(argc, argv, optstr, longopts, &optindex)) != -1) {
	...
	if (!strcmp(opt, "version")) {			/* --version */
	  ...
	  do_create_tables = 1;
	else if (!strcmp(opt, "create-cloudflare-tables"))
	  do_create_cf_tables = 1;
	  ...
	  run_as_root = 1;
	else if (!strcmp(opt, "host2"))
	  conf_set(&Conf, "db-host2", optarg, 0);
	else if (!strcmp(opt, "host3"))
	  conf_set(&Conf, "db-host3", optarg, 0);
	else if (!strcmp(opt, "host4"))
	  conf_set(&Conf, "db-host4", optarg, 0);
	else if (!strcmp(opt, "host-policy"))
	  conf_set(&Conf, "db-host-policy", optarg, 0);
