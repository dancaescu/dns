...
/**************************************************************************************************
	DB_OUTPUT_CLOUDFLARE_TABLES
	Output CREATE TABLE statements for Cloudflare mirror tables.
**************************************************************************************************/
void
db_output_cloudflare_tables(void) {
  puts("--");
  puts("--  Cloudflare mirror tables");
  puts("--");

  puts("CREATE TABLE IF NOT EXISTS `cloudflare_accounts` (");
  puts("  `id` int NOT NULL AUTO_INCREMENT,");
  puts("  `cf_account_id` varchar(64) NOT NULL,");
  ...
  puts("CREATE TABLE IF NOT EXISTS `cloudflare_zones` (");
  puts("  `id` int NOT NULL AUTO_INCREMENT,");
  puts("  `account_id` int NOT NULL,");
  puts("  `cf_zone_id` varchar(64) NOT NULL,");
  puts("  `name` varchar(255) NOT NULL,");
  puts("  `status` varchar(32) DEFAULT NULL,");
  puts("  `paused` tinyint(1) DEFAULT '0',");
  puts("  `zone_type` varchar(32) DEFAULT NULL,");
  puts("  `plan_name` varchar(128) DEFAULT NULL,");
  puts("  `favorite` tinyint(1) NOT NULL DEFAULT '0',");
  ...
  puts("CREATE TABLE IF NOT EXISTS `cloudflare_records` (");
  puts("  `id` bigint NOT NULL AUTO_INCREMENT,");
  puts("  `zone_id` int NOT NULL,");
  puts("  `cf_record_id` varchar(64) NOT NULL,");
  puts("  `record_type` varchar(16) NOT NULL,");
  puts("  `name` varchar(255) NOT NULL,");
  puts("  `content` text NOT NULL,");
  puts("  `ttl` int DEFAULT NULL,");
  puts("  `proxied` tinyint(1) DEFAULT NULL,");
  puts("  `priority` int DEFAULT NULL,");
  puts("  `data` longtext,");
  ...
  puts("CREATE TABLE IF NOT EXISTS `cloudflare_load_balancers` (");
  puts("  `id` int NOT NULL AUTO_INCREMENT,");
  puts("  `zone_id` int NOT NULL,");
  puts("  `cf_lb_id` varchar(64) NOT NULL,");
  puts("  `name` varchar(255) NOT NULL,");
  puts("  `proxied` tinyint(1) DEFAULT NULL,");
  puts("  `enabled` tinyint(1) DEFAULT NULL,");
  puts("  `fallback_pool` varchar(64) DEFAULT NULL,");
  puts("  `default_pools` text,");
  puts("  `steering_policy` varchar(32) DEFAULT NULL,");
  puts("  `data` longtext,");
  puts("  `created_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP,");
  puts("  `updated_at` timestamp NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,");
  puts("  PRIMARY KEY (`id`),");
