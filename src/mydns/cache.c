...
add_reply_to_cache(TASK *t) {
  ...
  if (ReplyCache->expire) {
    uint32_t min_ttl = ReplyCache->expire;
    unsigned char *rp = t->reply;
    DNS_HEADER hdr;
    uint16_t an = 0, ns = 0, ar = 0;

    if (t->replylen >= sizeof(DNS_HEADER)) {
      memcpy(&hdr, rp, sizeof(DNS_HEADER));
      rp += sizeof(DNS_HEADER);
      DNS_GET16(an, rp);
      DNS_GET16(ns, rp);
      DNS_GET16(ar, rp);
      rp = t->reply + sizeof(DNS_HEADER);
	...
	for (i = 0; i < counts[s]; i++) {
	  uchar *cur = (uchar*)t->reply + offset;
	  task_error_t errcode = 0;
	  if (offset >= t->replylen)
	    break;
	  if (!name_unencode2((uchar*)t->reply, t->replylen, &cur, &errcode))
	    break;
	  offset = cur - (uchar*)t->reply;
	  if (offset + SIZE16 * 2 > t->replylen)
	    break;
	  uint16_t type, class;
	  uchar *ptr = (uchar*)t->reply + offset;
	  DNS_GET16(type, ptr);
	  offset += SIZE16;
	  ptr = (uchar*)t->reply + offset;
	  DNS_GET16(class, ptr);
	  offset += SIZE16;
	  if (offset + SIZE32 + SIZE16 > t->replylen)
	    break;
	  uint32_t ttl;
	  ptr = (uchar*)t->reply + offset;
	  DNS_GET32(ttl, ptr);
    ...
    n->expire = current_time + MIN(ReplyCache->expire, min_ttl ? min_ttl : ReplyCache->expire);
  }
